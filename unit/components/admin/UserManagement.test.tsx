    ...
    <div data-testid="user-table">
      ...
      {users?.map((user: any) => (
        <div key={user.id} data-testid={`user-row-${user.id}`}>
          ...
          <button onClick={() => onToggleActive(user.id, !user.is_active)} data-testid={`toggle-${user.id}`}>
            Toggle
          </button>
      ...
      <button onClick={(e: any) => onSelectAll(e.target.checked)} data-testid="select-all">
        Select All
      </button>
    </div>
  ),
...
jest.mock('@/components/admin/UserImport', () => ({
  UserImport: ({ importing, importResult, onImport }: any) => (
    <div data-testid="user-import">
      ...
      <span data-testid="import-result">{importResult?.success ? 'success' : 'none'}</span>
      <button onClick={() => onImport([{ email: 'test@example.com' }])} data-testid="import-btn">
        Import
      </button>
    </div>
  ),
