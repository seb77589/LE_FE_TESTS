...
function createJsonResolveActivities<T>(activities: T): () => Promise<{ activities: T }> {
...
function createSuccessResponse<T>(
  jsonResolver: () => Promise<T>
): { ok: true; json: () => Promise<T> } {
...
jest.mock('@/components/admin/activity', () => {
  ...
  return {
    ActivityFilters: ({ filters, onFiltersChange, activityTypes, severityLevels }: any) =>
      React.createElement('div', { 'data-testid': 'activity-filters' },
        React.createElement('span', { 'data-testid': 'filter-hours' }, filters.hours),
        React.createElement('span', { 'data-testid': 'activity-types-count' }, activityTypes?.length),
        React.createElement('span', { 'data-testid': 'severity-levels-count' }, severityLevels?.length),
    ...
    ActivitySummary: ({ summary }: any) =>
      React.createElement('div', { 'data-testid': 'activity-summary' },
        React.createElement('span', { 'data-testid': 'summary-total' }, summary?.total_activities),
        React.createElement('span', { 'data-testid': 'summary-period' }, summary?.period_hours)
      ),
    ActivityList: ({ activities, searchTerm }: any) =>
      React.createElement('div', { 'data-testid': 'activity-list' },
        React.createElement('span', { 'data-testid': 'list-count' }, activities?.length),
        React.createElement('span', { 'data-testid': 'list-search' }, searchTerm),
        ...(activities?.map((a: any) =>
          React.createElement('div', { key: a.id, 'data-testid': `activity-${a.id}` }, a.action)
        ) || [])
      ),
...
describe('RealTimeActivityFeed', () => {
  ...
  const mockActivities = [
    { id: '1', action: 'user_login', user_email: 'user1@example.com', status: 'success', timestamp: '2025-01-01T12:00:00Z', severity: 'low' },
    { id: '2', action: 'document_uploaded', user_email: 'user2@example.com', status: 'success', timestamp: '2025-01-01T12:01:00Z', severity: 'medium' },
    { id: '3', action: 'security_event', user_email: 'user3@example.com', status: 'warning', timestamp: '2025-01-01T12:02:00Z', severity: 'high' },
  ];
  ...
  describe('error handling', () => {
    ...
    it('should call fetch when Retry button is clicked', async () => {
      let callCount = 0;
      const errorResponse = createErrorResponse(500, textResolveError);
      const successResponse = createSuccessResponse(createJsonResolveActivities(mockActivities));
  ...
  describe('edge cases', () => {
    it('should handle missing user_email with user_id fallback', async () => {
      mockFetch.mockResolvedValue({
        ...
        json: () => Promise.resolve({
          activities: [
            { id: '1', action: 'test', user_id: 123, status: 'success', timestamp: '2025-01-01T12:00:00Z' },
          ],
        }),
      });
    ...
    it('should display dash when no user info available', async () => {
      mockFetch.mockResolvedValue({
        ...
        json: () => Promise.resolve({
          activities: [
            { id: '1', action: 'test', status: 'success', timestamp: '2025-01-01T12:00:00Z' },
          ],
        }),
      });
