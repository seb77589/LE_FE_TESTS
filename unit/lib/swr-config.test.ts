...
// Mock useAuth hook
// Note: getValidAccessToken was removed in v0.2.0 HttpOnly cookie migration
// Authentication now uses HttpOnly cookies (credentials: 'include')
// isAuthenticated is checked before making requests
jest.mock('@/lib/context/ConsolidatedAuthContext', () => ({
  useAuth: () => ({
    isAuthenticated: true, // Default to authenticated for fetcher tests
  }),
}));
...
describe('SWR Config Module', () => {
  ...
  describe('baseSWRConfig', () => {
    it('should have revalidateOnFocus disabled', async () => {
      const { baseSWRConfig } = await import('@/lib/swr-config');
  ...
  describe('adminSWRConfig', () => {
    ...
    it('should inherit from baseSWRConfig', async () => {
      const { adminSWRConfig, baseSWRConfig } = await import('@/lib/swr-config');
  ...
  describe('realTimeSWRConfig', () => {
    it('should have refreshInterval of 5 seconds', async () => {
      const { realTimeSWRConfig } = await import('@/lib/swr-config');
  ...
  describe('streamingSWRConfig', () => {
    it('should return config with polling enabled when not streaming', async () => {
      const { streamingSWRConfig } = await import('@/lib/swr-config');
  ...
  describe('staticSWRConfig', () => {
    it('should have refreshInterval of 5 minutes', async () => {
      const { staticSWRConfig } = await import('@/lib/swr-config');
  ...
  describe('documentSWRConfig', () => {
    it('should have refreshInterval of 15 seconds', async () => {
      const { documentSWRConfig } = await import('@/lib/swr-config');
  ...
  describe('optimisticUpdates', () => {
    describe('updateUserInList', () => {
      it('should update matching user in list', async () => {
        const { optimisticUpdates } = await import('@/lib/swr-config');
        const users = [
          { id: 1, name: 'Alice' },
          { id: 2, name: 'Bob' },
          { id: 3, name: 'Charlie' },
  ...
  describe('useAuthenticatedFetcher', () => {
    it('should return fetcher functions', async () => {
      const { useAuthenticatedFetcher } = await import('@/lib/swr-config');
    ...
    // Note: Testing unauthenticated state requires module re-initialization
    // which is complex in Jest. The implementation is straightforward:
    // if (!isAuthenticated) throw new Error('Authentication required')
    // E2E tests validate this behavior in real browser context.
    // See: tests/e2e/auth-errors.spec.ts for comprehensive auth error testing

    // HttpOnly cookie migration (v0.2.0): Tokens are now in HttpOnly cookies
    // Requests use credentials: 'include' instead of Authorization header
    it('authenticatedFetcher - should make authenticated GET request with credentials', async () => {
  ...
  describe('default export', () => {
    it('should export baseSWRConfig as default', async () => {
      const defaultExport = (await import('@/lib/swr-config')).default;
      const { baseSWRConfig } = await import('@/lib/swr-config');
