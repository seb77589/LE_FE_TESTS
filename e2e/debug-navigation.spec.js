...
test.describe('LegalEase Debug Navigation as Manager', () => {
  ...
  test('Login and navigate all pages', async ({ page }) => {
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(SCREENSHOT_DIR, '02b-after-login.png'),
        fullPage: true,
      });
    }
    ...
    for (const pageInfo of pagesToVisit) {
      ...
      try {
        ...
        const screenshotPath = path.join(
          SCREENSHOT_DIR,
          `${String(pageIndex).padStart(2, '0')}-${pageInfo.name}.png`,
        );
        ...
        console.log(
          `  Buttons found: ${buttons.filter((b) => b.trim()).join(', ') || 'none'}`,
        );
        ...
        const errorMessages = await page
          .locator(
            '.error, .alert-error, [role="alert"], .text-red-500, .text-destructive',
          )
          .allTextContents();
        ...
        await page.screenshot({
          path: path.join(
            SCREENSHOT_DIR,
            `${String(pageIndex).padStart(2, '0')}-${pageInfo.name}-error.png`,
          ),
        });
        pageIndex++;
      }
