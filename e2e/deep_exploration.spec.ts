...
class DeepExplorer {
  ...
  private setupListeners() {
    ...
    this.page.on('console', (msg) => {
      ...
      if (type === 'error' && text.includes('403')) {
        console.log(
          `ℹ️ [${this.currentPage}] Expected 403 (MANAGER role): ${text.substring(0, 100)}`,
        );
        return;
      }
      ...
      if (type === 'error') {
        ...
        console.log(
          `❌ [${this.currentPage}] Console Error: ${text.substring(0, 200)}`,
        );
  ...
  async login(email: string, password: string): Promise<boolean> {
    ...
    try {
      ...
      await this.page
        .waitForSelector('[data-hydrated="true"]', { timeout: 10000 })
        .catch(() => {});
  ...
  async explorePage(url: string, name: string, depth: number = 0): Promise<void> {
    ...
    try {
      ...
      console.log(
        `${indent}   📊 Elements: ${buttonCount} buttons, ${linkCount} links, ${inputCount} inputs`,
      );
      ...
      for (let i = 0; i < Math.min(buttons.length, 2); i++) {
        try {
          ...
          if (!isDisabled && !isDangerous) {
            console.log(
              `${indent}   🖱️ Clicking button: "${text.trim().substring(0, 30)}"`,
            );
      ...
      const dropdowns = await this.page
        .locator(
          '[role="menubutton"], [aria-haspopup="true"], button[class*="dropdown"]',
        )
        .all();
      for (const dropdown of dropdowns.slice(0, 3)) {
        try {
          ...
          console.log(
            `${indent}   📋 Opening dropdown: "${text.trim().substring(0, 30)}"`,
          );
          ...
          for (const item of menuItems) {
            ...
            console.log(
              `${indent}      - Menu item: "${itemText?.trim().substring(0, 30)}"`,
            );
          }
      ...
      for (let i = 0; i < Math.min(inputs.length, 3); i++) {
        try {
          ...
          console.log(
            `${indent}   📝 Input: type=${type}, name=${name}, placeholder="${placeholder.substring(0, 30)}"`,
          );
        } catch (error) {
          // Continue
        }
...
test.describe('Deep Application Exploration', () => {
  ...
  test('Explore all pages as MANAGER', async ({ page }) => {
    ...
    const fs = require('fs');
    fs.writeFileSync(
      'test-results/exploration-issues.json',
