...
const ASSISTANT_EMAIL = process.env.TEST_ASSISTANT_EMAIL || 'manual-assistant@legalease.com';
...
test.describe('Manager Template RBAC - View & Modify', () => {
  ...
  test('Manager should see Edit button on company templates', async ({ page }) => {
    // Wait for templates to load
    await page.waitForSelector('[data-template-card], [data-template-row]', { timeout: 5000 });
  ...
  test('Manager can click template card to open view modal', async ({ page }) => {
    ...
    await expect(page.locator('[role="dialog"] button:has-text("Download")')).toBeVisible();
    await expect(page.locator('[role="dialog"] button:has-text("Use Template")')).toBeVisible();
    
    console.log('✅ Template view modal opens with correct actions');
  });

  test('Manager can see Modify button in view modal for company templates', async ({ page }) => {
...
test.describe('Manager Template RBAC - Modify Flow', () => {
  ...
  test('Manager can open modify modal from Edit button', async ({ page }) => {
    ...
    if (await companyTemplate.count() > 0) {
      ...
      await page.waitForSelector('[role="dialog"]:has-text("Modify Template")', { timeout: 3000 });
  ...
  test('Manager can update template metadata', async ({ page }) => {
    ...
    if (await companyTemplate.count() > 0) {
      ...
      // Update description
      await page.locator('textarea[name="description"]').fill('Updated description for E2E test');
...
test.describe('Manager Template RBAC - Create Flow', () => {
  ...
  test('Manager can open create modal', async ({ page }) => {
    ...
    await page.waitForSelector('[role="dialog"]:has-text("Create Template")', { timeout: 3000 });
...
test.describe('Assistant Template RBAC - Read-Only', () => {
  ...
  test('Assistant can view templates in read-only mode', async ({ page }) => {
    ...
    await expect(page.locator('[role="dialog"] button:has-text("Modify")')).not.toBeVisible();
...
test.describe('Template Browser Component RBAC', () => {
  ...
  test('Manager sees Edit buttons in Template Browser', async ({ page }) => {
    ...
    if (await browseTemplates.count() > 0) {
      ...
      await page.waitForSelector('[role="dialog"]:has-text("Browse Templates")', { timeout: 3000 });
...
test.describe('Permission Boundaries', () => {
  test('Manager cannot modify system templates (enforced by backend)', async ({ page }) => {
    await login(page, MANAGER_EMAIL, MANAGER_PASSWORD);
    await navigateToTemplates(page);
  ...
  test('Assistant cannot access modify endpoints directly (enforced by backend)', async ({ page }) => {
    await login(page, ASSISTANT_EMAIL, ASSISTANT_PASSWORD);
