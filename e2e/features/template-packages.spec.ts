...
test.describe('Template Package Mode Toggle', () => {
  ...
  test('should display mode toggle between Templates and Packages', async ({
    page,
  }) => {
  ...
  test('should switch to Packages mode when Packages button clicked', async ({
    page,
  }) => {
    ...
    const packageContent = await page
      .locator('[data-package-card], [data-package-row]')
      .count();
    const packageEmptyState = await page
      .locator(':has-text("Browse Template Packages")')
      .count();
  ...
  test('should switch back to Templates mode', async ({ page }) => {
    ...
    const templateContent = await page
      .locator('[data-template-card], [data-template-row]')
      .count();
    const templateEmptyState = await page
      .locator(':has-text("Browse Templates")')
      .count();
    ...
    console.log(
      `✓ Switched back to Templates mode (${templateContent} templates visible)`,
    );
  });
});
...
test.describe('Package Browser - Browse and Select', () => {
  ...
  test('should open package browser when Browse button clicked', async ({ page }) => {
    ...
    const browserDialog = page.locator(
      '[role="dialog"]:has-text("Browse Template Packages")',
    );
  ...
  test('should display package cards with metadata', async ({ page }) => {
    ...
    // Get first package
    const firstPackage =
      packageCards.count() > 0 ? packageCards.first() : packageRows.first();

    // Verify package displays key information
    const hasName = (await firstPackage.locator('h3').count()) > 0;
    const hasTemplateCount =
      (await firstPackage.locator(':has-text("template")').count()) > 0;
  ...
  test('should open preview modal when package clicked', async ({ page }) => {
    ...
    const hasTemplateList =
      (await previewModal.locator(':has-text("Included Templates")').count()) > 0;
    const hasUseButton =
      (await previewModal.locator('button:has-text("Use This Package")').count()) > 0;
...
test.describe('Package Variable Form - Fill and Submit', () => {
  ...
  test('should display merged variables from all templates', async ({ page }) => {
    ...
    const packageWithButton = page
      .locator(
        '[data-package-card]:has(button:has-text("Use Package")), [data-package-row]:has(button:has-text("Use Package"))',
      )
      .first();
    ...
    const inputFields = page.locator(
      '[role="dialog"] input[type="text"], [role="dialog"] input[type="number"], [role="dialog"] input[type="date"], [role="dialog"] textarea, [role="dialog"] select',
    );
  ...
  test('should show package information in form', async ({ page }) => {
    ...
    if ((await useButton.count()) > 0) {
      ...
      const hasPackageInfo =
        (await formDialog
          .locator(':has-text("Package Details"), :has-text("Included Templates")')
          .count()) > 0;
  ...
  test('should validate required variables', async ({ page }) => {
    ...
    if ((await useButton.count()) > 0) {
      ...
      const submitButton = page.locator(
        '[role="dialog"] button:has-text("Create Case"), [role="dialog"] button[type="submit"]',
      );
...
test.describe('Batch Case Creation from Package', () => {
  ...
  test('should create case when form submitted with valid data', async ({ page }) => {
    ...
    const caseTitleInput = page
      .locator(
        '[role="dialog"] input[name="case_title"], [role="dialog"] input[placeholder*="case title" i]',
      )
      .first();
    ...
    const textInputs = page.locator(
      '[role="dialog"] input[type="text"]:not([name="case_title"])',
    );
    ...
    console.log(
      '✓ Package variable form filled successfully (submission skipped to avoid test data)',
    );
  });
  ...
  test('should create multiple documents for multi-template package', async ({
    page,
  }) => {
...
test.describe('Package RBAC - Role-Based Access', () => {
  test('Managers should see Create Package button', async ({
    page,
    workerCredentials,
  }) => {
    ...
    const createButton = page.locator(
      'button:has-text("Create Package"), button:has-text("Create Template Package")',
    );

    // May or may not be visible depending on UI implementation
    const buttonCount = await createButton.count();
    console.log(
      `${buttonCount > 0 ? '✓' : '⚠'} Create Package button ${buttonCount > 0 ? 'found' : 'not found'} for Manager/SuperAdmin`,
    );
  });

  test('Assistants should NOT see Create Package button', async ({
    page,
    workerCredentials,
  }) => {
  ...
  test('All roles should be able to browse and use packages', async ({
    page,
    workerCredentials,
  }) => {
    await TestHelpers.loginAndWaitForRedirect(
    ...
    const packageCards = await page
      .locator('[data-package-card], [data-package-row]')
      .count();
    const browseButton = await page
