...
class ApplicationTester {
  ...
  private setupPageListeners() {
    ...
    this.page.on('requestfailed', (request) => {
      ...
      const isExpectedAbort =
        url.includes('frontend-errors/report') &&
  ...
  async login(email: string, password: string): Promise<boolean> {
    ...
    try {
      ...
      await this.page.screenshot({
        path: `test-results/${loginScreenshot}`,
        fullPage: true,
      });
  ...
  private async testInteractiveElements(pageName: string): Promise<void> {
    ...
    try {
      ...
      console.log(
        `   📊 Found ${buttons.length} buttons, ${links.length} links, ${inputs.length} inputs`,
      );
  ...
  generateReport(): void {
    ...
    // Summary
    const totalIssues =
      this.testResults.pageErrors.length +
