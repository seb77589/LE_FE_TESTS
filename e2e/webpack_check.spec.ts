...
async function loginAndNavigate(
  page: any,
  targetPath: string,
): Promise<{ webpackErrors: string[]; pageErrors: string[] }> {
...
test.describe('Webpack Module Check', () => {
  ...
  test('Cases page should load without webpack errors', async ({ page }) => {
    ...
    const hasContent =
      (await page
        .locator(
          '[data-testid="cases-container"], h1:has-text("Cases"), [data-testid="empty-state"]',
        )
        .count()) > 0;
    const hasError = (await page.locator('text=Try again').count()) > 0;
    ...
    await page.screenshot({
      path: 'test-results/cases-webpack-test.png',
      fullPage: true,
    });
